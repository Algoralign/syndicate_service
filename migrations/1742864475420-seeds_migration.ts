...
export class SeedsMigration1742864475420 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`
            INSERT INTO "countries" ("name", "continent", "dial_code", "value", "created_at", "updated_at")
            VALUES 
                ${countryData.map(
            country => `('${country.name}', '${country.continent}', '${country.dial_code}', '${country.value}', NOW(), NOW())`
        ).join(', ')}
        `);
    }
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`
            TRUNCATE TABLE "investment_instruments" RESTART IDENTITY CASCADE;
        `);
        ...
        await queryRunner.query(`
            TRUNCATE TABLE "identity_types" RESTART IDENTITY CASCADE;
        `);

        await queryRunner.query(`
            TRUNCATE TABLE "banks" RESTART IDENTITY CASCADE;
        `);

        await queryRunner.query(`
            TRUNCATE TABLE "schedule_periods" RESTART IDENTITY CASCADE;
        `);

        await queryRunner.query(`
            TRUNCATE TABLE "system_receiving_accounts" RESTART IDENTITY CASCADE;
        `);


        // Rollback the insert by deleting the inserted countries
        await queryRunner.query(`
            DELETE FROM "countries" WHERE "value" IN (${countryData.map(country => `'${country.value}'`).join(', ')});
        `);
