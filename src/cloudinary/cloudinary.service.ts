...
export class CloudinaryService {
    ...
    async uploadUserDocument(data, email: string): Promise<string> {
        try {
            ...
            const result = await cloudinary.uploader.upload(storagePath,
                { folder: 'syndicates_files/kycs/' + email, access_mode: 'public', resource_type: resourceType },);
    ...
    async uploadUseeDealDocument(data, email: string): Promise<string> {
        try {
            cloudinary.config({
                cloud_name: process.env.CLOUDINARY_CLOUD_NAME,
                api_key: process.env.CLOUDINARY_API_KEY,
            ...
            const storagePath = join(path.resolve('./'), `/uploads/deals-document/${data.filename}`);
            ...
            const result = await cloudinary.uploader.upload(storagePath,
                { folder: 'syndicates_files/deals/' + email, access_mode: 'public', resource_type: resourceType },);
    ...
    async uploadUseePaymentDocument(data, email: string): Promise<string> {
        try {
            cloudinary.config({
                cloud_name: process.env.CLOUDINARY_CLOUD_NAME,
                api_key: process.env.CLOUDINARY_API_KEY,
            ...
            const storagePath = join(path.resolve('./'), `/uploads/payment-document/${data.filename}`);
            ...
            const result = await cloudinary.uploader.upload(storagePath,
                { folder: 'syndicates_files/payments/' + email, access_mode: 'public', resource_type: resourceType },);
