        ...
        try {
            ...
            return await entityManager.transaction(async (transactionalEntityManager: EntityManager) => {
                ...
                const selfInvite = this.invitationTrackerRepository.create({
                    ...
                    currency: details.currency,
                    proposed_amount: details.investing_amount ? Number(details.investing_amount) : 0.00,
                    allocation_size: details.allocation_size ? Number(details.allocation_size) : 0.00,
                    invited_by: { id: userExist.id },
                    deal: { id: createdDeal.id },
                    syndicate: { id: theSyndicate.id },
                    ...
                    investment_fee_on_proposed_amount: fee,
                    proposed_amount_plus_investment_fee: (fee + (details.investing_amount ? Number(details.investing_amount) : 0.0)),
                    email_sent: true,
                    logged_in: true,
